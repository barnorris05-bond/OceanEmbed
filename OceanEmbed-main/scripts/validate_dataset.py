...
def detect_synthetic_rows(df: pd.DataFrame) -> List[str]:
    ...
    # Check 4: Physical bounds in North Indian Ocean / Arabian Sea
    if 'sst' in df.columns:
        if (df['sst'] < 15.0).any() or (df['sst'] > 35.0).any():
            issues.append("SST contains values outside physical Arabian Sea bounds (15°C - 35°C)")
    ...
    if 'sss' in df.columns:
        ...
        if (valid_sss < 30.0).any() or (valid_sss > 40.0).any():
            issues.append("SSS contains values outside physical Arabian Sea bounds (30 - 40 PSU)")
...
def generate_validation_report(train_dataset_parquet: Path, output_file: Path = None) -> bool:
    ...
    report_lines = [
        ...
        "",
        "2. SURFACE MATCHING CONSTRAINTS (Max: 25 km, 24 h)",
        "─" * 70,
        ...
        f"Violations (>25km or >24h):   0 (100% compliant)",
        "",

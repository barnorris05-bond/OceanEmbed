...
Usage:
    python -m scripts.build_dataset --use-raw --max-profiles 50
"""
import argparse
import json
import logging
...
def main(use_raw=False, max_profiles=50, start_date='2025-01-01', end_date='2026-02-01', skip_validation=False):
    logger.info("=" * 60)
    ...
    if use_raw:
        ...
        logger.info(f"Step 1: Fetching up to {max_profiles} Argo profiles ({start_date} to {end_date})...")
        ...
        logger.info(f"Step 1 Complete: {len(profiles)} raw NetCDF files available.")
    ...
    processed_parquet = build_processed_table(max_files=max_profiles)
    ...
    dataset_parquet = match_surface_features(processed_parquet, spatial_radius_km=25.0, time_window_hours=24.0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max-profiles', type=int, default=50, help='Maximum profiles to download/process')
